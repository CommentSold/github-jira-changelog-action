...
class SourceControl {
  ...
  getCommitLogs(workspaceDir, range, accesstoken, repoName) {
    ...
    return new Promise((resolve, reject) => {
      ...
      workspace.log(opts, (err, response) => {
        ...
        const promises = logs.map(log => {
          const prNumber = this.getPullRequestNumber(log.summary);

          await sleep(2000);

          if(prNumber && prNumber.length > 0) {
            const pullRequestNumber = prNumber[0].substr(2, prNumber[0].length - 3);
            //console.log("Pul lrequest number: ", pullRequestNumber);
            return this.prInfoPromise(client, pullRequestNumber, repoName).catch(err => {
              console.log(err);
            }) // ignore errors
            .then(prInfo => {
              if (prInfo) {
                log.fullText = log.fullText + " PR BODY:  " + prInfo.body + " PR TITLE: " + prInfo.title;
                log.prInfo = prInfo;
              }
              return log;
            });
  ...
  getPullRequestNumber(commitMessage) {
    const prPattern = new RegExp(/\(#([0-9]+)\)/, 'g');
    // console.log('PR match: ', commitMessage.match(prPattern));
    return commitMessage.match(prPattern) || [];
  }

  prInfoPromise(ghClient, prNumber, repoName) {
    return new Promise((resolve, reject) => {
      const ghpr = ghClient.pr(repoName, prNumber);
