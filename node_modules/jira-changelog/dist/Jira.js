...
class Jira {
  ...
  findJiraInCommit(commitLog) {
    ...
    tickets.forEach(ticketMatch => {
      ...
      if (key == 'WEB-100' || key == 'ARCH-741') { // this is in our PR template, skip it.
        return;
      }
  ...
  addTicketsToReleaseVersion(tickets, versionName) {
    ...
    return _asyncToGenerator(function* () {
      ...
      tickets = tickets.filter((ticket) => {
        return !(ticket.fields.status.name == "Deployed" || ticket.fields.status.name == "Completed" || ticket.fields.status.name == "Done. Follow up?" || ticket.fields.status.name == "Closed" || ticket.fields.status.name == 'Needs Feature Testing' || ticket.fields.status.name == 'Ready to Enable' || ticket.fields.status.name == 'Deployed and Eanbled');
      });
